...
class PyNCCLDistributedImpl(DistributedImpl):
    ...
    # the .cu dtype map covers fp16/bf16 only; anything else must take the torch fallback
    _PYNCCL_DTYPES = (torch.float16, torch.bfloat16)

    def _check_dtype(self, x: torch.Tensor) -> None:
        if x.dtype not in self._PYNCCL_DTYPES:
            raise TypeError(
                f"PyNCCL communicator does not support {x.dtype} tensors; "
...
def destroy_distributed() -> None:
    """
    Destroy all the distributed communication plugins, restoring the plain
    torch.distributed fallback: plugins[-1] dispatch breaks if the list is empty.
    """
